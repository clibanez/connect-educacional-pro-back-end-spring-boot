...
public class ResourceExceptionHandler {
    ...
    public ResponseEntity<StandardError> handleMissingServletRequestPartException(MissingServletRequestPartException ex,
                                                                                  HttpServletRequest request) {

        StandardError error = new StandardError(System.currentTimeMillis(), HttpStatus.BAD_REQUEST.value(),
                "Missing Servlet Request Part", ex.getMessage(), request.getRequestURI());
        error.setMessage("A FOTO é obrigatória!");
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);

    }

    //Metodo para retrição de autorização do usuário
    ...
    public ResponseEntity<StandardError> handleAccessDeniedException(AccessDeniedException ex,
        ...
        error.setMessage("Você Não tem autorização!");
...
//    public ResponseEntity<StandardError> duplicateEntryException(DuplicateEntryException ex,
//                                                                 HttpServletRequest request) {
//        StandardError error = new StandardError(System.currentTimeMillis(), HttpStatus.CONFLICT.value(),
//                "Duplicate Entry tetetstetstetse", ex.getMessage(), request.getRequestURI());
//
